...
#heredem funcionalitats de la classe pare    
...
class InventoryScreen(MDScreen):
    ...
    def buscar(self, item):
        ...
        dataDispositius = app.on_start()
        
        search_device = [search_field for search_field in dataDispositius if item.lower() in search_field['id_inventory'].lower()]
        
        #actualitzar la llista filtrada
        search_device_list = self.root.ids.list
        search_device_list.clear_widgwts()
        
        for result in search_device:
            search_device_list.add_widget(
                OneLineIconListItem(
                    IconLeftWidget(
                            icon="laptop"
                        ), 
